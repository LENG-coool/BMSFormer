...
## 实验表现{#实验表现}
通过在**Oxford、NASA 和 CALCE**三大权威数据集上的验证，BMSFormer验证了其在电池寿命预测上的有效性：

- **估算精度提升**：在所有数据集上，其误差指标（MAE和RMSE）均为最低。相比传统LSTM模型，平均误差降低了 47% 至 73% 。
 
- **拟合的优度高**：其R²评分在各场景下均最接近 1（最高达 0.9934），证明其预测值与真实健康状态最为贴合。
